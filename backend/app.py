...
# Keep existing registration and login endpoints unchanged...
...
def register():
    ...
    try:
        ...
        if not re.match(mobile_pattern, data['mobile']):
...
def get_profile(current_user_id):
    ...
    try:
        ...
        recent_reports = list(complaints_collection.find(
            {'email': user['email']},
            {'_id': 1, 'category': 1, 'description': 1, 'status': 1, 'created_at': 1}
...
# UPDATED SUBMIT REPORT ENDPOINT - Enhanced for Anonymous Reporting
@app.route('/api/submit-report', methods=['POST'])
@verify_token(optional=True)
def submit_report(current_user_id=None):
    """Submit a new complaint/report - supports both authenticated and anonymous users"""
    try:
        ...
        if is_anonymous:
            logger.info(f"Processing anonymous report for category: {data.get('category')}")
        else:
            logger.info(f"Processing authenticated report from user: {current_user_id}")
        
        # Validate required fields
        required_fields = ['name', 'surname', 'email', 'mobile', 'category', 'description']
        for field in required_fields:
            if not data.get(field) or not str(data.get(field)).strip():
                return jsonify({'error': f'{field} is required'}), 400
        ...
        # Validate category
        valid_categories = ['pothole', 'streetlight', 'sewer', 'garbage', 'water_supply', 'road_repair', 'other']
        ...
        # Use ML prediction if category is missing or "other"
        if (not data.get("category") or data["category"] == "other") and predicted_category:
            data["category"] = predicted_category
            logger.info(f"ML Predicted category: {predicted_category}")
        ...
        complaint_data = {
            ...
            'department': department_map.get(data['category'], 'General'),
            ...
            'is_anonymous': is_anonymous,  # NEW: Track anonymous reports
            'upvotes': 0,
            'downvotes': 0,
            'comments': []
        }
        ...
        return jsonify({
            ...
            'ml_prediction': predicted_category if predicted_category else None
        }), 201
        
    except Exception as e:
        logger.error(f"Submit report error: {e}")
        return jsonify({'error': 'Report submission failed. Please try again.'}), 500

# UPDATED GET REPORTS ENDPOINT - Filter anonymous reports in public view
...
# NEW ENDPOINT - Get Anonymous Report Statistics
@app.route('/api/anonymous-stats', methods=['GET'])
def get_anonymous_stats():
    """Get statistics about anonymous reporting"""
    try:
        total_reports = complaints_collection.count_documents({})
        anonymous_reports = complaints_collection.count_documents({'is_anonymous': True})
        authenticated_reports = total_reports - anonymous_reports
        
        # Category breakdown for anonymous reports
        anonymous_categories = list(complaints_collection.aggregate([
            {'$match': {'is_anonymous': True}},
            {'$group': {'_id': '$category', 'count': {'$sum': 1}}},
            {'$sort': {'count': -1}}
        ]))
        
        # Recent anonymous reports (last 7 days)
        week_ago = datetime.utcnow() - timedelta(days=7)
        recent_anonymous = complaints_collection.count_documents({
            'is_anonymous': True,
        ...
        return jsonify({
            ...
            'stats': {
                ...
                'authenticated_reports': authenticated_reports,
                'anonymous_percentage': round((anonymous_reports / max(total_reports, 1)) * 100, 2),
                'recent_anonymous_weekly': recent_anonymous
            },
            'anonymous_categories': anonymous_categories
        }), 200
        
    except Exception as e:
        logger.error(f"Anonymous stats error: {e}")
        return jsonify({'error': 'Failed to fetch anonymous statistics'}), 500

# Keep all other existing endpoints unchanged...
# (get_report_details, vote_report, get_map_data, get_leaderboard, etc.)
...
@verify_token()
def vote_report(current_user_id, report_id):
    ...
    try:
        ...
        # Award points to report creator (only for authenticated reports)
        if report.get('user_id') and not report.get('is_anonymous'):
            points = 2 if vote_type == 'up' else -1
            users_collection.update_one(
        ...
        return jsonify({
            'success': True,
            'message': f'Report {vote_type}voted successfully'
        }), 200
        
    except Exception as e:
        logger.error(f"Vote report error: {e}")
        return jsonify({'error': 'Failed to vote on report'}), 500
...
def get_stats():
    """Get application statistics with anonymous reporting data"""
    try:
        # Basic stats
        total_reports = complaints_collection.count_documents({})
        submitted_reports = complaints_collection.count_documents({'submitted': 1})
        resolved_reports = complaints_collection.count_documents({'status': 'resolved'})
        pending_reports = complaints_collection.count_documents({'status': 'pending'})
        anonymous_reports = complaints_collection.count_documents({'is_anonymous': True})
        authenticated_reports = total_reports - anonymous_reports
        total_users = users_collection.count_documents({'is_active': True})
        
        # Category breakdown
        category_stats = list(complaints_collection.aggregate([
            {'$group': {'_id': '$category', 'count': {'$sum': 1}}},
            {'$sort': {'count': -1}}
        ]))
        ...
        report_type_stats = [
            {'_id': 'anonymous', 'count': anonymous_reports},
            {'_id': 'authenticated', 'count': authenticated_reports}
        ]
        
        # District breakdown
        district_stats = list(complaints_collection.aggregate([
        ...
        # Status breakdown
        status_stats = list(complaints_collection.aggregate([
            {'$group': {'_id': '$status', 'count': {'$sum': 1}}},
            {'$sort': {'count': -1}}
        ]))
        
        # Priority breakdown
        priority_stats = list(complaints_collection.aggregate([
        ...
        monthly_stats = list(complaints_collection.aggregate([
            ...
            {'$group': {
                ...
                'authenticated_count': {
                    '$sum': {'$cond': [{'$ne': ['$is_anonymous', True]}, 1, 0]}
                }
            }},
        ...
        return jsonify({
            ...
            'stats': {
                ...
                'anonymous_percentage': round((anonymous_reports / max(total_reports, 1)) * 100, 2),
                'resolution_rate': round((resolved_reports / max(total_reports, 1)) * 100, 2)
            },
            'breakdowns': {
                'categories': category_stats,
                'report_types': report_type_stats,
...
# Keep remaining endpoints unchanged (categories, districts, search, nearby-reports, health, admin/dashboard, error handlers)
...
def health_check():
    ...
    return jsonify({
        ...
        'version': '2.1.0',
        'features': ['anonymous_reporting', 'ml_prediction', 'auto_submission']
    }), 200
...
if __name__ == '__main__':
    print("=" * 70)
    print("🚀 Starting CivicFix Flask Application v2.1.0")
    print("=" * 70)
    ...
    print("├─────────────────────────────────────────────────────────────────────┤")
    print("│ Reports Management (Enhanced for Anonymous)                         │")
    print("├─────────────────────────────────────────────────────────────────────┤")
    print("│ POST   /api/submit-report     - Submit complaint (auth/anonymous)   │")
    print("│ GET    /api/reports           - Get all reports (filter anonymous)  │")
    print("│ GET    /api/reports/<id>      - Get report details                  │")
    print("│ POST   /api/reports/<id>/vote - Vote on report                      │")
    print("│ GET    /api/search            - Search reports                      │")
    print("│ GET    /api/nearby-reports    - Get nearby reports                  │")
    print("├─────────────────────────────────────────────────────────────────────┤")
    print("│ Data & Analytics (Updated)                                          │")
    print("├─────────────────────────────────────────────────────────────────────┤")
    print("│ GET    /api/map-data          - Get map data                        │")
    print("│ GET    /api/leaderboard       - Get leaderboard                     │")
    print("│ GET    /api/stats             - Get statistics (with anonymous)     │")
    print("│ GET    /api/anonymous-stats   - Get anonymous report statistics     │")
    print("│ GET    /api/categories        - Get categories                      │")
    print("│ GET    /api/districts         - Get districts                       │")
    print("├─────────────────────────────────────────────────────────────────────┤")
    print("│ Admin & System                                                      │")
    print("├─────────────────────────────────────────────────────────────────────┤")
    ...
    print("🎯 New Features: Anonymous reporting, ML category prediction")
    print("=" * 70)
